function [ ind_err, ind_std ] = get_EnsembleErrorMean( dirName, y )
...
for i = 1 : length(noises)
    for j = 1 : length(corruptionRates)
        ...
        for a = 1 : length(initializations)
            ...
            error = 0;
            for c = 1 : size(y_prob,3)
                [~, ypred] = max(y_prob(:,:,c),[],2);
                bad = find(ypred ~= expected); 
                error(c) = numel(bad) / length(y);
            end
            ind_err(i,j) = mean(error);
            ind_std(i,j) = std(error);
        end
    end
end
