function [ individualError, ensembleSTD, noises, corruptionRates ] = get_IndividualError( dirName, y )
...
corruptionRates = [0,0.4, 0.5, 0.6, 0.7];
    ...
    for j = 1 : length(corruptionRates)
        ...
        for a = 1 : length(initializations)
            ...
            for n = 1 : numModels
                ...
                probComb = y_prob(:,:,comb);
                %ensemble
                [~, ensemble_y] = max(sum(probComb,3),[],2);
                bad = find(ensemble_y ~= expected);    
                ensembleError{i,j}(n) = numel(bad) / length(y);
            end
        end
    end
end
