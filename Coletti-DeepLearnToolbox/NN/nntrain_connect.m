...
for k = 1 : length(nn.epochSchedule)
    ...
    for i = currEpoch : numepochs
        ...
        disp(['epoch ' num2str(i) '/' num2str(maxEpochs) '. Took ' num2str(t) ' seconds' '. Mini-batch mean squared error on training set is ' num2str(mean(L((n-numbatches):(n-1)))) '. LR: ', num2str(nn.learningRate) str_perf]);
